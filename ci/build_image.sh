...
: "${GITHUB_USER:=ppdeploy-user}"
: "${DEPLOY_USER:=$GITHUB_USER}"
...
export DEPLOY_USER DEPLOY_TOKEN
# Pass any extra arguments to the build command ("$@" contains the rest of the arguments)
set -x
$runtime build --tag "$full_tag" "$@" \
               --secret=id=deploy_user,src=DEPLOY_USER,type=env \
               --secret=id=deploy_token,src=DEPLOY_TOKEN,type=env \
